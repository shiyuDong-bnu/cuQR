...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    std::cout<<"omp duration: "<<duration<<"ms ";

    // begin naive part to compare
    double trans_array2[dim2][dim1];
    for (int i=0;i<dim2;i++){
        for (int j=0;j<dim1;j++){
            trans_array2[i][j]=A.at(j,i);
        }
    }
    double betas[dim2];
    start=std::chrono::high_resolution_clock::now();
    serial_qr((double *)trans_array2,dim1,dim2,betas);
    end=std::chrono::high_resolution_clock::now();
    duration=std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count();
    std::cout<<"naive duration: "<<duration<<"ms"<<std::endl;
    return 0;
}
void house( double x[],int size , double v[],double & beta){
